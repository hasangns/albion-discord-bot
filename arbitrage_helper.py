...
async def arbitrage(item_tier, item_name):
    url = 'https://west.albion-online-data.com/api/v2/stats/Prices/' + \
        str(item_tier).upper() + '_' + str(item_name).upper() + '.json'
    print(url)
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as response:
            ...
            for item in json_data:
                if 'city' in item and 'sell_price_min' in item and 'sell_price_min_date' and 'quality' in item:
                    city = item['city']
                    ...
                    if int(price) != 0:
                        if city == 'Black Market':
                            bm_data.append(
                                (price, new_quality, city, price_date))
                        if city != 'Black Market':
                            others_data.append(
                                (price, new_quality, city, price_date))
            # Check profit
            for i in bm_data:
                for k in others_data:
                    if i[1] == k[1] and i[0] - k[0] > 0:
                        ...
                        results.append(
                            (bm_price, others_price, others_city, qualities, profit, bm_date, others_date))
            if results:
                return results
            else:
                return None
